...
public class IT473MarkUpdate extends AppCompatActivity {
    ...
    public  void update(View v){
        ...
        final  int q = Integer.parseInt(quizMark);
        String midMark = mid.getText().toString().trim();
        final int m = Integer.parseInt(midMark);
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                dataSnapshot.getRef().child("quiz").setValue(q);
                dataSnapshot.getRef().child("mid").setValue(m);
                dataSnapshot.getRef().child("finals").setValue(f);
